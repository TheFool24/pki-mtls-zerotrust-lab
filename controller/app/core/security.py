"""mTLS identity extraction from Nginx-forwarded headers.
...
subject DN as headers. This module extracts and validates that identity.

Headers (set by nginx mtls.conf):
    X-Client-Verify: SUCCESS | NONE | FAILED:<reason>
    X-Client-DN:     CN=pi-01.thesis.local

CN parsing: extracts the common name from the DN string.
Example DN: "CN=controller.thesis.local"
"""
import re
